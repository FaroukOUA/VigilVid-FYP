...
- Phase 7 Solo Mode foundation is working with backend-randomized public
  Hugging Face Dataset clips, bundled fallback clips, local progress tracking,
  and signed-in score persistence.
- Phase 7 Man vs Machine local flow is working with model predictions from the MintVid evaluation export.
- The Game tab calls FastAPI for randomized game clips from the public Hugging
  Face Dataset; Expo does not fetch Hugging Face Dataset files directly.
...
- Game clip playback uses backend proxy URLs and conservative backend
  transcoding so clips are prepared as Android-playable MP4. The backend
  prepares the first selected clip before returning the round and warms the
  remaining selected clips in the background.
