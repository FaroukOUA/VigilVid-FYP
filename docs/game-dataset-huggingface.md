...
- Backend game clip serving uses `GAME_CLIP_TRANSCODE_MODE=auto`.
...
uses `GAME_CLIP_TRANSCODE_MODE=auto`: it serves phone-safe H.264/yuv420p clips
directly and transcodes only risky source encodings. During local development,
set `GAME_CLIP_LOCAL_EXPORT_ROOT` to the unzipped export folder so the backend
does not have to download each game clip from Hugging Face.
